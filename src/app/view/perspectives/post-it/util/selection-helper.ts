...
export class SelectionHelper {
  ...
  private tryToSelectDocumentOnUp(newColumn: number, newRow: number): void {
    const selectedDocument = this.postIts.find(postIt => postIt.order === this.selectedPostIt.order - this.getDocumentsPerRow());
    this.select(newColumn, newRow, selectedDocument);
  }
  ...
  private selectedInputId(): string {
    return `${ this.perspectiveId }${ this.selectedPostIt.index }[${ this.selection.column }, ${ this.selection.row }]`;
  }
